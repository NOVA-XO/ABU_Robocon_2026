 ...
 */
void Drive_Distance_Start(
    long counts); // урагш явах зай (encoder тоо; + = урагш)
uint8_t
Drive_Distance_Service(void);   // давталт бүрд дуудна: 1=дуусав, 0=явж байна
...
void Gyro_Strafe_Test(
    void);                // рак 600 + ✕→1сек зүүн/1сек баруун (gyro чиг барих)
...
int Rack_Preset_Buttons(
    void); // рак preset товчны логик (OLED-гүй), буц: зорилт
int Servo_Preset_Buttons(void); // серво товчны логик (OLED-гүй), буц: өнцөг

/* ---- Серво ----------------------------------------------------------------
 */
#define SERVO_DEG_MAX 180 // сервоны бүтэн зам (градус)
#define SERVO_DEG_MIN 0   // сервоны хамгийн доод өнцөг

#define SERVO_HOME_DEG SERVO_DEG_MIN // АСААХАД энэ байрлалаас эхэлнэ
...
typedef struct {
  ...
  uint8_t land_soft; // 1 = ЗӨӨЛӨН газардалт (шугаман удаашрал), 0 = ХҮЧТЭЙ
                     // (бүрэн pwm_down)
  ...
  uint32_t last_step_ms; // сүүлд rack_step дуудагдсан агшин (команд аваагүй =
                         // идэвхгүй)
 ...
 *
 *  Switch-ийг рак бүр өөрөө home_sw талбартаа агуулна (enc индексээр БҮҮ
 * сонго).
 * -----------------------------------------------------------------------------
 */
...
uint8_t Rack_SetHome(
    Rack_t *r); // limit switch хүртэл доошлуулж 0 цэг тогтооно
...
uint8_t
Rack_GoTo_Sync(int target); // ХОЁР ракыг ХАМТ, зэрэгцүүлж; хоёул хүрвэл 1
uint8_t
Rack_GoTo_Sync_Front(int base,
                     int front_offset); // синхрон, front нь +offset өндөр
...
void Robot_Error(
    const char *msg); // жолоо зогс, рак 0,0 руу буулга, зогс (буцдаггүй)

/* Rack удирдлага (Rack_SetHome-ын ДАРАА) */
void Rack_Climb_Test(
    void); // L1/R1→2уул 0/1000,  L2/△→front 0/1000,  R2/✕→back 0/1000
void Rack_Preset_Control(
    void); // L1→0  L2→900  R1→1350  R2→1950 (хоёулаа хамт; тогтмол preset)

/* Серво (1 серво, htim1 CH1), ГРАДУСААР — асаахад 180° */
void Servo_Preset_Control(
    void); // Cross +5°  Triangle −5°  Circle→0°  Square→180°
...
/* Тааруулгын телеметр: UART4 (115200) руу TSV —
 * timer/tgt/pos/pwm/integral/switch */
...
int Rack_PWM_Tune(
    uint8_t motor);            // motor 5=front / 6=back; буцаана: одоогийн PWM
