...
/* -----------------------------------------------------------------------------
 *  sendEncoderSerial — Encoder 0 ба 1-ийн утгыг UART4-ээр (115200) илгээх
 *
 *    Мөр бүрийн формат:   E0:<утга> E1:<утга>\r\n
 *    ENC_TX_MS тутам НЭГ удаа илгээнэ — send_uart* нь блоклодог (HAL_UART_Transmit)
 *    тул давталт бүрд илгээвэл гогцоог бөглөж, дэлгэц/удирдлага удааширна.
 *
 *  counter[] нь EXTI ISR-т өөрчлөгддөг ч 32-bit int-ийн НЭГ уншилт Cortex-M4
 *  дээр атомик тул критик муж шаардлагагүй.
 *
 *  ⚠ send_uart* нь huart4 ашигладаг. LPMS gyro МӨН huart4 дээр тул LPMS_Init()
 *    идэвхжсэн үед энэ функцийг БҮҮ дууд (протокол зөрчилдөнө).
 * -----------------------------------------------------------------------------
 */
#define ENC_TX_MS  100   // илгээх интервал (мс)

void sendEncoderSerial(void) {
    static uint32_t t = 0;

    if (HAL_GetTick() - t < ENC_TX_MS) return;
    t = HAL_GetTick();

    send_uart_int(counter[0]);
    send_uart("\t");
    send_uart_int(counter[1]);
    send_uart("\r\n");
}
