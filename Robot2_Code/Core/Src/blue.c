...
#define WB_DRIVE_SLOW                                                          \
  (-100) // намуухан урагш (val8 руу дөхөх — хэт хурдан байв)
#define WB_RACK_BASE_LOW                                                       \
  1400 // урагш явахдаа BACK рак ЭНД (FRONT нь +offset өндөр)
#define WB_FRONT_OFFSET                                                        \
  0 // FRONT/BACK ТЭГШ (өмнө 100 байсан — front өндөр явдгийг болиулав)
#define WB_RACK_UP                                                             \
  1950 // val5 мэдэрч ЗОГССОНЫ дараа хоёр рак ЭНД дээшилнэ.
       //   pos_max(2000)-аас доош тул барихад timeout болохгүй.
#define WB_RACK_FINAL 1800 // 180° эргэсний ДАРАА рак хоёулаа ЭНД (бүтэн) очно.
...
#define WB_VAL8_HOLD_MS                                                        \
  200 // val8 ийм хугацаанд ТАСРАЛТГҮЙ 0 байвал "шахагдсан"
...
#define WB_ALIGN_KP                                                            \
  8.0f // gyro чиг барих коэффициент (Drive_Straight-тэй ижил маяг)
...
typedef enum {
  ...
  WB_S3_PUSH_VAL8, // намуухан урагш + серво ЗЭРЭГ буулгах, val8 ТАСРАЛТГҮЙ 0
                   // болтол
  WB_S4_ALIGN_VAL3, // val3==1 бол зүүн тийш (gyro+val8), val3==0 бол → 1сек
                    // хүлээх
  ...
  WB_S7_RACK,      // эргэсний ДАРАА рак хоёулаа 1900 руу (бүтэн)
  WB_DONE
} wb_state_t;
...
static int wb_rack_base =
    WB_RACK_BASE_LOW; // BACK зорилт (урагш 1400 → val5 1950)
static int wb_front_off =
    WB_FRONT_OFFSET; // FRONT-ийн нэмэлт өндөр (урагш 50 → val5 0)
 ...
 *    S4: val3==1 бол → val3==0 болтол ЗҮҮН тийш (gyro+val8).  val3==0 бол →
 * S4B. S4B: val3==0 → 500мс хүлээгээд → соленоид1 ON. S4C: 500мс хүлээгээд →
 * серво 180°. S5: 700мс УХРАХ (шулуун). S6: 180° эргэх → дуусна.
 *
...
uint8_t weapon_blue(void) {
   ...
   *  Аль ч төлөвт рак идэвхгүй үлдвэл (рак_step дуудагдахгүй)
   * cmd_target/move_t0 хөлдөж, open-loop сажирч ХУДАЛ timeout болдог. Тиймээс
   * ЭНД нэг л удаа, бүх төлөвт дуудна. Target нь эхнээс эцэс хүртэл 1950
   * (тогтмол). (мотор 5,6 — жолоо/эргэлтийн мотор 1-4-тэй зөрчилдөхгүй.) */
  ...
  case WB_S3_PUSH_VAL8:
    ...
    if (val8 == 0) {
      if (wb_v8_t0 == 0)
        wb_v8_t0 = HAL_GetTick(); // 0 болсон эхлэл
  ...
  case WB_S6_TURN:
    ...
    if (Gyro_TurnAngle(WB_TURN_DEG)) { // дуусвал 1 (дотроо LPMS_Read)
      brake();
      wb_rack_base = WB_RACK_FINAL; // эргэсэн → рак хоёулаа 1900 руу
      wb_front_off = 0;             // FRONT/BACK тэгш
      wb_st = WB_S7_RACK;
    }
    break;

  /* -------- S7: 180° эргэсний ДАРАА рак хоёулаа 1900 руу → ДУУССАН -------- */
  case WB_S7_RACK:
    brake();
    if (rack_done) // Rack_GoTo_Sync_Front(1900, 0)-д хүрэв
      wb_st = WB_DONE;
    break;

  /* -------- ДУУССАН / БАРИХ: урагш зогссон, рак 1900-д барина -------- */
  case WB_DONE:
