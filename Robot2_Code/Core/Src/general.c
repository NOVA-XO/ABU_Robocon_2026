 ...
 *              (mecanum жолоо, rack PID, gyro эргэлт/тэгшлэлт, налуу
 * гарах/буух)
 *
...
static int g_servo_deg =
    SERVO_HOME_DEG; // АСААХАД home-оос (0°) эхэлнэ; Servo_Home() hardware-т бичнэ
 ...
 *  pos_max-ийг encoder-ийн CPR-аар дахин хэмжиж бич (Rack_Jog +
 * showEncoderStatus).
 * =============================================================================
 */
 ...
 *    дээр (зорилтоос 104 зайд) ГАЦДАГ байв — телеметрт Fpwm яг 190 дээр
 * тогтсон. Front нь 261+ PWM дээр хөдөлдөг тул тагийг өндөрсгөв.
 *
 ...
 *  hold_pwm нь таталцлын ҮНДСЭН хүчийг өгнө; integral зөвхөн ҮЛДЭГДЛИЙГ трим
 * хийнэ. RACK_I_BAND — хураах бүс. Өргөн байвал ойртох замдаа хэт их хурааж
 * ХАНАНА. RACK_I_DEAD — encoder шумаас integral мөлхөхөөс сэргийлэх үхмэл бүс
 * (count).
 */
#define RACK_I_BAND                                                            \
  80 // үүнээс ХОЛ бол integral-ыг тэглэнэ (windup-аас сэргийлнэ)
     // >= RACK_NEAR_ZONE (60) байх ЁСТОЙ — дээрх урхийг үзнэ үү
/*  RACK_I_DEAD — ҮРЭЛТИЙН (stiction) бүсийг бүрхэх ёстой. Механизм өөрөө
 * түгжигддэг: телеметрээр back rack нь 10..170 PWM хооронд ОГТ хөдөлдөггүй нь
 * батлагдсан. Хэрэв энэ бүсэд integral-ыг үргэлжлүүлэн хураавал рак хөдлөхгүй
 * байхад integral −150 хүртэл ханаж, PWM-ийг 10 болтол буулгаад ч гэсэн рак
 * хөдөлдөггүй (хий ажил).
 */
 ...
 *  RACK_LAND_MIN_PWM → switch дээр газардах үеийн PWM (хэт бага бол хүрэлгүй
 * зогсоно)
 */
#define RACK_LAND_ZONE 600 // 0-оос энэ зайд орохоос эхлэн удаашруулна (count)
#define RACK_LAND_MIN_PWM                                                      \
  50 // газардах агшны хамгийн бага PWM (governor-ийн ШАЛ)
...
/* 0 руу (home) буухдаа switch-ийг НАЙДВАРТАЙ цохих доод хурд. Ойрын бүсэд
   kp·алдаа бага болж жижиг pwm-ээр switch дэргэд гацахаас сэргийлнэ (homing-тэй
   ижил хүч). Switch хүрэхгүй бол reached=0 → Rack_Fault timeout хамгаална. */
#define RACK_HOME_APPROACH 250

/* ---- Аюулгүй байдал (Rack_Fault) — ТЕЛЕМЕТРЭЭР ТААРУУЛСАН -----------------
 *  Хэмжилт (50мс/мөр): бүтэн зам 0→1950 нь ~25 мөр ≈ 1250мс. Өгсөх үед
 * front/back зөрүү дээд тал нь ~70 count. Тиймээс: TIMEOUT = 1250мс + ~2x нөөц
 * = 2500мс (гацсан ракыг ~2.5с-д барина, худал триггергүй; батарей суларвал ч
 * бүтэн зам үүнээс богино). SYNC_MAX = хэвийн ~70-ийн ~4x = 300 (нэг рак гацвал
 * зөрүү үүнийг хурдан давна; хэвийн 70 зөрүүнд худал триггер өгөхгүй). */
#define RACK_TIMEOUT_MARGIN                                                    \
  60 // timeout нь ЗӨВХӨН рак зорилтоос ИЙМ ХОЛ (жинхэнэ гацсан)
     //   үед л асна. tolerance (15)-ийн ирмэг дээр тогтвол
     //   (ж: 1933) "хүрээгүй" гэж timeout болохоос сэргийлнэ.
#define RACK_TIMEOUT_MS                                                        \
  6000 // зорилтод хүрэх / БАРИХ дээд хугацаа; хэтэрвэл алдаа
...
#define RACK_SYNC_MAX                                                          \
  450 // Rack_Fault-ийн ХЯЗГААР (count) — синхрончлол дампуурвал барина
#define RACK_SYNC_TIMEOUT_MS                                                   \
  1500 // зөрүү ТАСРАЛТГҮЙ ийм удаан > SYNC_MAX байвал л алдаа
       //   (агшны биш): state шилжихэд түр зуурын зөрүү ХУДАЛ асахгүй
#define RACK_SYNC_BAND                                                         \
  150 // Rack_GoTo_Sync: хоёр рак нэг нэгнээсээ хамгийн ихдээ энэ зайд.
      //   ⚠ RACK_NEAR_ZONE (60)-аас ИХ байх ёстой: бага бол өгсөх бүх
      //   зуур "ойрын хурд хязгаар"-т баригдаж рак УДААН мөлхөнө
      //   (20 үед weapon_blue-д урагш явахад рак өгсөж амжихгүй байв).
#define RACK_SYNC_NEAR                                                         \
  100 // sync шалгах: зорилтоос энэ дотор бол "хүрсэн/parked" гэж үзнэ
#define RACK_IDLE_MS                                                           \
  200 // rack_step сүүлд дуудагдснаас хойш энэ хугацаа өнгөрвөл рак
      //   КОМАНД АВААГҮЙ (идэвхгүй) — Rack_Fault түүнийг шалгахгүй
      //   (командгүй байхад cmd_target/move_t0 хуучирч ХУДАЛ timeout).
...
Rack_t frontRack = {
    .motor = 6,
    .enc = 1,
    .home_sw = RACK_SW_S2, // ХЭМЖСЭН: M6 / E1 / S2
    .pos_min = 0,
    .pos_max = 2000, // ДЭЭД хязгаар — Rack_SetTarget зорилтыг үүн рүү хайчилна
    // FRONT нь back-аас ХОЛ ИЛҮҮ АЧААТАЙ (телеметрээр: 900-г барихад back 69
    // PWM,
    //   FRONT 173 PWM хэрэгтэй). Тиймээс:
    //   - kp-г 2.0 → 3.5 (back-аас өндөр): өгсөх/ойртохдоо хоцрохгүй,
    //   хангалттай хүч.
    //   - hold_pwm-ийг 70 → 150: барих хүчийг feedforward-оор ШУУД өгнө; өмнө
    //   нь
    //     integral 100 хүртэл УДААН ханаж байж нөхдөг тул мөлхөж хоцордог байв.
    .kp = 3.5f,
    .ki = 0.012f,
    .kd = 28.0f, // 18→28: FRONT тогтмол +14 хальдаг байв — илүү хүчтэй тоормос
    .i_max = 12500.0f, // kd 9→18→28: зорилтод давахыг тоормослох
    // pwm_up 750→700: back (650)-тай тэнцүүлэв. 750 үед front БАГА ЗЭРЭГ
    // түрүүлж
    //   байсан тул арай удаашруулав (back-ийн 650-д ойртуулав).
    ...
    .min_pwm = 80,
    .tolerance = 15,
    .land_soft = 1, // default: зөөлөн газардалт
    .target = 0,
    .active = 0,
    .prev_err = 0.0f,
    .integral = 0.0f,
...
Rack_t backRack = {
    .motor = 5,
    .enc = 0,
    .home_sw = RACK_SW_S1, // ХЭМЖСЭН: M5 / E0 / S1
    .pos_min = 0,
    .pos_max = 2000, // ДЭЭД хязгаар — Rack_SetTarget зорилтыг үүн рүү хайчилнаs
    // frontRack-тай ижил зарчим: удаан, ханахгүй integral (0.012 x 12500 = 150
    // PWM дээд)
    .kp = 3.0f,
    .ki = 0.012f,
    .kd = 18.0f,
    .i_max = 12500.0f, // kd 10→18: давахыг тоормослох
    // hold_pwm — back rack-ийн үрэлтийн бүс 10..170 PWM (телеметрээр
    // батлагдсан) тул
    //   түүний ДУНДАЖ. 175 нь бүсээс ДЭЭГҮҮР байсан → байнга дээш түлхэж
    //   гацаадаг байв.
    // pwm_down 350→600: back ГАНЦААРАА (land_soft=0) 0 руу буух хурдыг ихэсгэв.
    //   (Хоёулаа зэрэг буух land_soft=1 нь хурдны governor-оор намуухан
    //   хэвээр.)
    .pwm_up = 750,
    .pwm_down = 600,
    .pwm_soft = 350, // ЗӨӨЛӨН (хамтдаа) буултын ТУСДАА дээд PWM (pwm_down-оос
 ...
 *  ӨМНӨ нь switch-ийг enc индексээр сонгодог байсан нь урд/хойдыг СОЛЬЖ уншиж
 * байв.
 * -----------------------------------------------------------------------------
 */
 ...
 *  counter[] нь EXTI ISR-т read-modify-write (counter[i]--) хийгддэг. Main
 * loop-оос бичихэд EXTI дундуур таслаад хуучин утгаар дарж бичиж болзошгүй тул
 * богино критик мужаар хамгаална. (TIM7 ISR-ээс дуудахад ч аюулгүй — PRIMASK
 * сэргээнэ.)
 * -----------------------------------------------------------------------------
 */
 ...
 *
 *  БЛОКЛОДОГ: HAL_Delay ашигладаг тул ЗӨВХӨН main loop-оос дуудна (ISR-ээс
 * БИШ). Дуудах үед тухайн ракийн ISR-service-ийг түр унтраана (Rack_Service
 * зөрчихгүй).
 *
...
/* -----------------------------------------------------------------------------
 *  rack_step — Нэг ракийн PID-ийн НЭГ алхам (Rack_GoTo ба Rack_Service
 * хуваалцана) return: 1 = хүрсэн (tolerance дотор), 0 = хөдөлж байна ISR-safe:
 * зөвхөн HAL_GetTick, float, motor_control ашиглана.
 * -----------------------------------------------------------------------------
 */
static uint8_t rack_step(Rack_t *r, int target) {
  r->last_step_ms =
      HAL_GetTick(); // "рак командлагдаж байна" тэмдэг (Rack_Fault уншина)
  ...
  // ---- Алдаа хянах цаг: зорилт өөрчлөгдсөн ЭСВЭЛ хүрсэн үед timeout-ыг
  // тэглэнэ.
  ...
  //     - integral — ЖИНГ автоматаар нөхнө (gravity comp; жин нэмэгдвэл өөрөө
  //     өснө)
  //     - шүүсэн derivative — damping (encoder ±1 шумыг дарж савалгааг
  //     сэргийлнэ)
  if (HAL_GetTick() - r->last_ms >= RACK_DT_MS) {
    ...
    float output =
        (r->kp * error) + (r->ki * r->integral) + (r->kd * r->d_filt);

    // Таталцлын FEEDFORWARD — өргөгдсөн байрлалд суурь дээш bias.
    //   ⚠ ЗӨВХӨН рак зорилтдоо / доогуур байхад (error > -tolerance) нэмнэ.
    //   Рак зорилтоос ДЭЭШ давсан үед (буух шаардлагатай) дээш bias нэмвэл
    //   буухыг саатуулж дээр гацдаг тул тэр үед НЭМЭХГҮЙ.
    if (raised && error > -(float)r->tolerance)
      output += (float)r->hold_pwm;
    ...
    //   ЗАСВАР: land_soft=1 бол ЗӨВХӨН 0 руу биш, ДУНД ТҮВШИН рүү буухыг ч
    //   (1350→900 г.м.) governor-оор удаашруулна — бодит нөхцөлд хэт хурдан
    //   байсан. Тусад нь (land_soft=0) бол урьдын адил бүрэн pwm_down (хурдан).
    uint8_t landing = (error < 0.0f && r->land_soft);
    if (landing) {
      float speed = r->d_filt; // count/cycle (буух үед +)
      if (speed > RACK_DOWN_VMAX)
        r->down_lim -= RACK_DOWN_STEP; // хурдан → сааруул
      else
        r->down_lim += RACK_DOWN_STEP; // удаан → нэм
      ...
      if (aerr < (float)RACK_LAND_ZONE) { // switch рүү ойртвол нэмж зөөлрүүл
        int near = RACK_LAND_MIN_PWM +
                   (int)(((float)(land - RACK_LAND_MIN_PWM) * aerr) /
                         (float)RACK_LAND_ZONE);
        if (land > near)
          land = near;
      }
      if (lim > land)
    ...
    // min_pwm floor зөвхөн ТОМ зөрөөнд (хол хөдөлгөөн, статик үрэлт даван
    // гарах). Ойрын жижиг залруулгыг зөөлөн үлдээж хэлбэлзэхээс сэргийлнэ.
    // ГАЗАРДАХ үед floor хэрэглэхгүй — эс бөгөөс удаашруулах профайлыг эвдэнэ.
    if (aerr > (float)RACK_NEAR_ZONE && !landing) {
      if (pwm > 0 && pwm < r->min_pwm)
        pwm = r->min_pwm;
      if (pwm < 0 && pwm > -r->min_pwm)
        pwm = -r->min_pwm;
    }
    ...
    // ХАМГААЛАЛТ (дээд): pos_max хүрсэн бол ДЭЭШ (эерэг PWM) жолоодохгүй —
    // рак pos_max (2000)-аас ГАРАХААС сэргийлнэ. PID хэт хол зорьсон ч энд
    // таглана.
    if (pos >= r->pos_max && pwm > 0)
      pwm = 0;
 ...
 * -----------------------------------------------------------------------------
 */
uint8_t Rack_GoTo(Rack_t *r, int target) { return rack_step(r, target); }
...
uint8_t Rack_GoTo_Sync(int target) {
  ...
  int ftgt = target;
  if (ftgt > bpos + RACK_SYNC_BAND)
    ftgt = bpos + RACK_SYNC_BAND;
  if (ftgt < bpos - RACK_SYNC_BAND)
    ftgt = bpos - RACK_SYNC_BAND;

  int btgt = target;
  if (btgt > fpos + RACK_SYNC_BAND)
    btgt = fpos + RACK_SYNC_BAND;
  if (btgt < fpos - RACK_SYNC_BAND)
    btgt = fpos - RACK_SYNC_BAND;
...
/* -----------------------------------------------------------------------------
 *  Rack_GoTo_Sync_Front — Rack_GoTo_Sync-ийн адил СИНХРОН, гэхдээ FRONT ракыг
 *  BACK-аас front_offset-оор ӨНДӨР зорилттой барина.
 *
 *    base         : BACK ракийн эцсийн зорилт
 *    front_offset : FRONT нь base + front_offset руу (эерэг = front өндөр)
 *
 *  Урагш явахад front инерцээр бага зэрэг унаж (тонгойх) байвал front-ийг арай
 *  өндөр барьж нөхнө. Хоёр рак ХАМТ (leader-follower) хөдлөх тул зөрүү
 * тогтвортой. Band-ыг ХАРЬЦАНГУЙ байрлалд барина: (fpos − offset) нь bpos-тэй
 * зэрэгцэнэ.
 * -----------------------------------------------------------------------------
 */
uint8_t Rack_GoTo_Sync_Front(int base, int front_offset) {
  int fpos = counter[frontRack.enc];
  int bpos = counter[backRack.enc];

  int f_final = base + front_offset; // FRONT-ийн эцсийн зорилт
  int b_final = base;                // BACK-ийн эцсийн зорилт

  // FRONT-ийг (bpos + offset)-аас ±band; BACK-ийг (fpos − offset)-аас ±band
  int ftgt = f_final;
  if (ftgt > bpos + front_offset + RACK_SYNC_BAND)
    ftgt = bpos + front_offset + RACK_SYNC_BAND;
  if (ftgt < bpos + front_offset - RACK_SYNC_BAND)
    ftgt = bpos + front_offset - RACK_SYNC_BAND;

  int btgt = b_final;
  if (btgt > fpos - front_offset + RACK_SYNC_BAND)
    btgt = fpos - front_offset + RACK_SYNC_BAND;
  if (btgt < fpos - front_offset - RACK_SYNC_BAND)
    btgt = fpos - front_offset - RACK_SYNC_BAND;
...
void Rack_SetTarget(Rack_t *r, int target) {
  ...
  if (target != r->target) { // зорилт ШИНЭЭР өөрчлөгдсөн
    r->down_lim =
        (float)RACK_LAND_MIN_PWM; // буулт бол намуханаас эхэл (огцом уналгүй)
  }
  r->target = target;
  r->active = 1;
}
...
void Rack_Service(void) {
  if (frontRack.active)
    rack_step(&frontRack, frontRack.target);
  if (backRack.active)
    rack_step(&backRack, backRack.target);
}
...
uint8_t Rack_Fault(void) {
  ...
  static uint32_t to_bad_t0[2] = {0, 0}; // рак бүр зорилтоос ХОЛ болсон агшин
  static uint32_t sync_bad_t0 = 0; // зөрүү SYNC_MAX-аас хэтэрч эхэлсэн агшин
  Rack_t *racks[2] = {&frontRack, &backRack};

  /* ⚠ ЗӨВХӨН ХОЁР РАК НЭГЭН ЗЭРЭГ командлагдаж байхад л шалгана.
   *  Хоёулаа сүүлийн RACK_IDLE_MS дотор rack_step-д орсон байх ёстой (= хоёул
   *  идэвхтэй, хамт командлагдсан). Нэг нь идэвхгүй ЭСВЭЛ тус тусад нь
   *  командлагдсан (ж: sequence-д front/back ээлжлэн хөдлөх) үед ХУДАЛ алдаа
   *  өгдөг тул огт шалгахгүй.                                                */
  if ((now - frontRack.last_step_ms) > RACK_IDLE_MS ||
      (now - backRack.last_step_ms) > RACK_IDLE_MS) {
    to_bad_t0[0] = to_bad_t0[1] = 0;
    sync_bad_t0 = 0;
    return 0;
  }

  // 1) TIMEOUT — рак бүр: зорилтоос ХОЛ (>= MARGIN) ТАСРАЛТГҮЙ RACK_TIMEOUT_MS
  //    байвал л алдаа. MARGIN (60) ашиглана: рак зорилтдоо ойрхон (15..60)
  //    тогтвол "хүрсэн" гэж үзэж timeout болгохгүй. Мөн тасралтгүй таймер тул
  //    зорилтдоо байгаад агшин зуур үсрэхэд ХУДАЛ асахгүй (move_t0-оос
  //    хамаарахгүй).
  for (int i = 0; i < 2; i++) {
    Rack_t *r = racks[i];
    ...
    if (err < 0)
      err = -err;
    if (err >= RACK_TIMEOUT_MARGIN) {
      if (to_bad_t0[i] == 0)
        to_bad_t0[i] = now;
      if (now - to_bad_t0[i] > RACK_TIMEOUT_MS)
        return 1;
    } else {
      to_bad_t0[i] = 0; // ойртвол таймерыг тэглэнэ
    }
  }

  // 2) SYNC — хоёулаа ижил зорилт руу ХАМТ хол байхад зөрүү хэтэрвэл.
  //    ⚠ АГШИНД биш: зөрүү ТАСРАЛТГҮЙ RACK_SYNC_TIMEOUT_MS хэтэрвэл л асна.
  //    Ингэснээр нэг state-аас нөгөөд шилжихэд (ж: 1000 → 1000 дахин командлах)
  //    түр зуурын зөрүү ХУДАЛ алдаа өгөхгүй — рак дахин зэрэгцэж амжина.
  uint8_t sync_bad = 0;
  if (frontRack.cmd_target == backRack.cmd_target) {
    int ft = frontRack.cmd_target;
    int fe = counter[frontRack.enc] - ft;
    if (fe < 0)
      fe = -fe;
    int be = counter[backRack.enc] - ft;
    if (be < 0)
      be = -be;
  ...
  if (sync_bad) {
    if (sync_bad_t0 == 0)
      sync_bad_t0 = now;
    if (now - sync_bad_t0 > RACK_SYNC_TIMEOUT_MS)
      return 2;
  } else {
    sync_bad_t0 = 0; // зэрэгцвэл таймерыг тэглэнэ
  }
...
/* -----------------------------------------------------------------------------
 *  Rack_PWM_Tune — PS5 D-pad-аар тогтмол (open-loop) PWM-ийг бодит цагт
 * тааруулах
 *
 ...
 *  PWM-ийг encoder feedback-ГҮЙгээр шууд өгнө — таталцлыг бариулах утгыг
 * олоход, мөн савалгаа удирдлагынх уу (feedback) эсвэл механикийнх уу гэдгийг
 * ялгахад. Одоогийн PWM болон encoder утгыг OLED дээр харуулна.
 *
...
int Rack_PWM_Tune(uint8_t motor) {
  ...
  // Товч дарагдсан бол TUNE_RATE_MS тутам нэг алхам (тонших = нарийн, барих =
  // хурдан)
...
void Rack_Climb_Test(void) {
  static Btn_t bL1 = {0}, bR1 = {0}, bL2 = {0}, bR2 = {0}, bTri = {0},
               bCross = {0};
  ...
  if (btn_rising(&bL1,
                 (uint8_t)control_data[3][0])) { // L1 → ХОЁУЛАА доош (зөөлөн)
    tgtFront = RACK_LEVEL_DOWN;
    frontRack.land_soft = 1;
    tgtBack = RACK_LEVEL_DOWN;
    backRack.land_soft = 1;
  }
  ...
  if (btn_rising(
          &bL2,
          (uint8_t)control_data[3][2])) { // L2 → зөвхөн FRONT доош (хүчтэй)
    tgtFront = RACK_LEVEL_DOWN;
    frontRack.land_soft = 0;
  }
  if (btn_rising(
          &bR2,
          (uint8_t)control_data[3][3])) { // R2 → зөвхөн BACK доош (хүчтэй)
    tgtBack = RACK_LEVEL_DOWN;
    backRack.land_soft = 0;
  }
  /* --- Тусад нь ДЭЭШ: авираад буухад рак бүрийг ээлжлэн татахад --- */
  if (btn_rising(&bTri, (uint8_t)control_data[1][2])) { // △ → зөвхөн FRONT дээш
    tgtFront = RACK_LEVEL_UP;
  }
  if (btn_rising(&bCross,
                 (uint8_t)control_data[1][0])) { // ✕ → зөвхөн BACK дээш
    tgtBack = RACK_LEVEL_UP;
  }
...
int Rack_Preset_Buttons(void) {
  ...
  if (btn_rising(&bL1, (uint8_t)control_data[3][0]))
    target = RACK_CLIMB_L0; // 0
  if (btn_rising(&bL2, (uint8_t)control_data[3][2]))
    target = RACK_CLIMB_L1; // 900
  if (btn_rising(&bR1, (uint8_t)control_data[3][1]))
    target = RACK_CLIMB_L2; // 1350
  if (btn_rising(&bR2, (uint8_t)control_data[3][3]))
    target = RACK_CLIMB_L3; // 1950
...
void Rack_Preset_Control(void) {
  runner(); // стик → mecanum жолоо (мотор 1-4)
  int target =
      Rack_Preset_Buttons(); // L1/L2/R1/R2 → рак (мотор 5/6, ISR барина)
...
/* -----------------------------------------------------------------------------
 *  Servo_Home — эхлэлийн байрлалд (SERVO_HOME_DEG = 0°) тавих. Init-д ЗААВАЛ дуудна.
 *
 *  g_servo_deg нь home-оор эхэлдэг ч энэ нь зөвхөн ПРОГРАММЫН утга — hardware
...
void Rack_Telemetry_Serial(void) {
  ...
  if (!hdr) {
    send_uart("timer\tBtgt\tBpos\tBpwm\tBi\tFtgt\tFpos\tFpwm\tFi\tS1\tS2\r\n");
    hdr = 1;
  }

  if (HAL_GetTick() - t < RACK_TLM_MS)
    return;
  t = HAL_GetTick();

  char buf[128];
  sprintf(buf, "%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\r\n", timer,
          backRack.cmd_target, counter[backRack.enc], backRack.last_pwm,
          (int)(backRack.ki * backRack.integral), frontRack.cmd_target,
          counter[frontRack.enc], frontRack.last_pwm,
          (int)(frontRack.ki * frontRack.integral), (int)val1, (int)val2);
  send_uart(buf);
}

/* -----------------------------------------------------------------------------
 *  Rack_Solo_Test — СОЛО рак хөдөлгөөн турших (нэг рак дангаар дээш/доош):
 *    △ → FRONT дээш (SOLO_UP)   ✕ → FRONT доош (0)
 *    ○ → BACK  дээш (SOLO_UP)   ▭ → BACK  доош (0)
 *  land_soft = 0 (СОЛО/хатуу) — governor-гүй, дунд түвшний удаашрал БАЙХГҮЙ.
 *  Rack_Service (TIM7 ISR) байрлалд барина. UART4 телеметр + OLED.
 *  ⚠ UART4 ашигладаг тул main.c-д LPMS_Init() унтраасан байх ёстой.
 * -----------------------------------------------------------------------------
 */
#define RACK_SOLO_UP 1900 // соло тестийн дээш зорилт

void Rack_Solo_Test(void) {
  static Btn_t bTri = {0}, bCr = {0}, bCi = {0}, bSq = {0};
  static uint8_t inited = 0;

  if (!inited) {
    frontRack.land_soft = 0; // СОЛО: тусад нь буух — governor-гүй, хурдан
    backRack.land_soft = 0;
    inited = 1;
  }
  ...
  /* FRONT: △ дээш / ✕ доош */
  if (btn_rising(&bTri, (uint8_t)control_data[1][2]))
    Rack_SetTarget(&frontRack, RACK_SOLO_UP);
  if (btn_rising(&bCr, (uint8_t)control_data[1][0]))
    Rack_SetTarget(&frontRack, 0);
  /* BACK: ○ дээш / ▭ доош */
  if (btn_rising(&bCi, (uint8_t)control_data[1][3]))
    Rack_SetTarget(&backRack, RACK_SOLO_UP);
  if (btn_rising(&bSq, (uint8_t)control_data[1][1]))
    Rack_SetTarget(&backRack, 0);

  /* ⚠ Serial (Rack_Telemetry_Serial) ХАССАН — LPMS gyro UART4-ийг эзэлсэн.
     Соло рак хөдөлгөөнийг зөвхөн OLED-оор харна. */
  ...
  if (HAL_GetTick() - t >= 100) {
    ...
    printStr("RACK SOLO");
    setCursor(2, 22);
    printStr("F %d>%d", counter[frontRack.enc], frontRack.target);
    setCursor(2, 42);
    printStr("B %d>%d", counter[backRack.enc], backRack.target);
    setScreen();
  }
}

/* -----------------------------------------------------------------------------
 *  Rack_PWM_Tune_Dual — ХОЁР моторыг ТУСАД нь open-loop PWM тааруулах
 * (debounce-тай)
 *
 *    Мотор 5 (front): D-Up (control_data[2][2]) +,  D-Down (control_data[2][0])
 * − Мотор 6 (back) : Triangle (control_data[1][2]) +,  Cross
 * (control_data[1][0]) −
 *
 *  Мотор бүр өөрийн PWM-тэй, тусдаа тааруулагдана. OLED дээр M5/M6 хоёуланг
 * харуулна.
 * -----------------------------------------------------------------------------
 */
void Rack_PWM_Tune_Dual(void) {
  ...
  if (btn_step(&bUp, control_data[2][2]))
    pwm5 += TUNE_STEP; // D-Up
  if (btn_step(&bDn, control_data[2][0]))
    pwm5 -= TUNE_STEP; // D-Down

  // --- Мотор 6 (back): Triangle / Cross ---
  if (btn_step(&bTri, control_data[1][2]))
    pwm6 += TUNE_STEP; // Triangle
  if (btn_step(&bCr, control_data[1][0]))
    pwm6 -= TUNE_STEP; // Cross
 ...
 *
 *  OLED:
 *    M5/M6 = өгсөн PWM,   E0/E1 = counter,   S1/S2 = limit switch (0 =
 * ДАРАГДСАН) 4-р мөр: "5>E0+ 6>E1-"  ← мотор>encoder ба чиглэлийн тэмдэг ('?' =
 * хараахан үгүй)
 *
...
void Rack_Joystick_Test(void) {
   ...
   *  sw[m]     — тухайн мотор ажиллаж байхад ирмэг өгсөн limit switch (1=S1,
   * 2=S2)
   * ------------------------------------------------------------------------ */
  ...
  // --- Стикээс PWM. ХЭМЖСЭН: түүхий стик ДЭЭШ = ЭЕРЭГ тул эргүүлэхгүй шууд
  // авна ---
  int ly = control_data[0][1]; // зүүн стик Y
  int ry = control_data[0][3]; // баруун стик Y
  if (ly > -JTEST_DEADZONE && ly < JTEST_DEADZONE)
    ly = 0;
  if (ry > -JTEST_DEADZONE && ry < JTEST_DEADZONE)
    ry = 0;
  ...
  if (m >= 0) {
    ...
    if (s1 != ps1)
      sw[m] = 1; // энэ мотор явж байхад S1 ирмэг өглөө
    if (s2 != ps2)
      sw[m] = 2;
  }
  ps1 = s1;
  ...
  if (HAL_GetTick() - t >= 100) {
    ...
    if (enc[0] < 0)
      ...
      printStr("5>E%d%c ", enc[0], sgn[0]);
    if (enc[1] < 0)
      printStr("6>E??");
    else
      printStr("6>E%d%c", enc[1], sgn[1]);
    setScreen();
  }
  ...
  if (HAL_GetTick() - ts >= 250) {
    ...
    sprintf(buf,
            "M5>E%c dir:%c sw:%s | M6>E%c dir:%c sw:%s | E0:%d E1:%d S1:%d "
            "S2:%d\r\n",
...
#define TURN_D_ALPHA                                                           \
  0.6f // derivative шүүлтүүр (LPMS 90Hz, PID 10ms → чимээтэй)
...
#define TURN_RUNAWAY_DEG                                                       \
  30.0f // алдаа эхнийхээсээ ийм хэмжээгээр ӨСВӨЛ = тэмдэг буруу
...
uint8_t Gyro_TurnFail(void) { return turn_fail; }
...
uint8_t Gyro_TurnAngle(float angle) {
  ...
  LPMS_Read();
  float current =
      yaw_rel; // тасралтгүй (-360..+360), зөвхөн энэ эргэлтэд харьцангуй
  ...
  if (timer > TURN_DT_MS) {
    ...
    if (error < TURN_I_BAND && error > -TURN_I_BAND) {
      turn_integ += error;
      if (turn_integ > TURN_I_MAX)
        turn_integ = TURN_I_MAX;
      if (turn_integ < -TURN_I_MAX)
        turn_integ = -TURN_I_MAX;
    } else {
      turn_integ = 0.0f;
    ...
    float output =
        (TURN_KP * error) + (TURN_KI * turn_integ) + (TURN_KD * turn_dfilt);

    if (output > TURN_MAX_PWM)
      output = TURN_MAX_PWM;
    if (output < -TURN_MAX_PWM)
      output = -TURN_MAX_PWM;
...
void Drive_Straight_Test(void) {
  ...
  if (btn_rising(&bDown, (uint8_t)control_data[2][0])) {
    on = 0;
    brake();
  }
 ...
 *        S = сервоны өнцөг,  F/B = ракуудын бодит байрлал,  ms = эргэлтийн
 * хугацаа.
 * =============================================================================
 */
...
void Gyro_Turn_Test(void) {
  ...
  if (btn_rising(&bDown,
                 (uint8_t)control_data[2][0])) { // D-Down → яаралтай таслах
    Gyro_TurnReset();
    mode = MODE_JOY;
    brake();
  }
  ...
  if (HAL_GetTick() - t >= 100) {
    ...
    if (fail == 2)
      ...
      printStr("ms:%lu", (unsigned long)last_ms);
    setScreen();
  }
}

/* =============================================================================
 *  Gyro_Strafe_Test — рак 600-д өргөөд, gyro чиг барьж 1сек ЗҮҮН + 1сек БАРУУН
 *
 *    Эхлээд хоёр ракыг СИНХРОН 600-д өргөнө. Рак хүрмэгц ✕ (Cross) дарахад
 *    1 секунд зүүн, 1 секунд баруун хажуулна. Хажуулах зуур gyro-гоор чигээ
 *    (anchor) барина — эргэлдэхгүй шулуухан хажуу тийш явна.
 *
 *    ✕ (Cross) → зүүн-баруун мөчлөг эхлүүлэх (давтан дарж болно)
 *    OLED: st (төлөв), off (anchor-аас хазайлт °), F/B (рак байрлал)
 * =============================================================================
 */
#define GST_RACK 600   // ракийн өргөх байрлал
#define GST_STRAFE 250 // хажуулах хурд (PWM)
#define GST_MS 1000    // чиглэл тус бүрийн хугацаа (1 секунд)
#define GST_KP 8.0f    // gyro чиг барих коэффициент (Drive_Straight маягтай)
#define GST_WMAX 150   // gyro залруулгын дээд PWM

/* Хажуу тийш + gyro чиг барих (mecanum). vx < 0 → ЗҮҮН, vx > 0 → баруун. */
static void strafe_gyro(int vx) {
  LPMS_Read();
  float off = Get_Yaw_Offset_From_Anchor();
  int c = (int)(GST_KP * off);
  if (c > GST_WMAX)
    c = GST_WMAX;
  if (c < -GST_WMAX)
    c = -GST_WMAX;

  motor_control(1, -vx - c); // Урд-Зүүн
  motor_control(2, vx + c);  // Урд-Баруун
  motor_control(3, vx - c);  // Хойд-Зүүн
  motor_control(4, -vx + c); // Хойд-Баруун
}

/* Гаднаас дуудах strafe (auto_climb-ийн grab-д): vx>0 БАРУУН, vx<0 ЗҮҮН.
   Gyro-гоор чиг барина (Set_Yaw_Anchor-аас хойшхи хазайлт). */
void Strafe_Gyro(int vx) { strafe_gyro(vx); }

void Gyro_Strafe_Test(void) {
  static uint8_t st = 0; // 0=рак өргөх, 1=бэлэн, 2=зүүн, 3=баруун
  static uint32_t t0 = 0;
  static Btn_t bGo = {0};

  /* Рак 600-д бүх төлөвт СИНХРОН барина (мотор 5,6 — жолоотой зөрчилдөхгүй) */
  uint8_t rack_ok = Rack_GoTo_Sync(GST_RACK);

  switch (st) {
  case 0: // рак 600-д хүрэх хүртэл (жолоо зогссон)
    brake();
    if (rack_ok)
      st = 1;
    break;

  case 1: // бэлэн — ✕ дарахыг хүлээх
    brake();
    if (btn_rising(&bGo, (uint8_t)control_data[1][0])) { // ✕
      Set_Yaw_Anchor(); // одоогийн чигийг "шулуун" болгох
      t0 = HAL_GetTick();
      st = 2;
    }
    break;

  case 2: // 1 секунд ЗҮҮН
    if (HAL_GetTick() - t0 >= GST_MS) {
      t0 = HAL_GetTick();
      st = 3;
      break;
    }
    strafe_gyro(-GST_STRAFE);
    break;

  case 3: // 1 секунд БАРУУН
    if (HAL_GetTick() - t0 >= GST_MS) {
      brake();
      st = 1;
      break;
  ...
  if (HAL_GetTick() - t >= 100) {
    ...
    colorFill(Black);
    setCursor(2, 2);
    printStr("STRAFE st:%d", (int)st);
    setCursor(2, 22);
    printStr("off:%d", (int)Get_Yaw_Offset_From_Anchor());
    setCursor(2, 42);
    printStr("F:%d B:%d", counter[frontRack.enc], counter[backRack.enc]);
    setScreen();
  }
 ...
 *    дотор ESP32-оос багц бүрдэх бүрд АВТОМАТААР явагдана — горимоос
 * хамаарахгүй. Энд байт илгээвэл тэр урсгалын дундуур орж багцыг эвдэнэ.
 *
