...
void controlSolenoid(uint8_t solenoidNumber, bool state) {
  if (solenoidNumber < 1 || solenoidNumber > 8)
    errorFunction();

  GPIO_TypeDef *ports[] = {OP1_GPIO_Port, OP2_GPIO_Port, OP3_GPIO_Port,
                           OP4_GPIO_Port, OP5_GPIO_Port, OP6_GPIO_Port,
                           OP7_GPIO_Port, OP8_GPIO_Port};
  ...
  HAL_GPIO_WritePin(ports[solenoidNumber - 1], pins[solenoidNumber - 1],
                    pinState);
}
...
void controlServo(bool isServo, int angle) {
  if (angle > 24000 || angle < 0)
    errorFunction();

  if (isServo)
    __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, 24000 + angle);
  else
    __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_3, 24000 + angle);
}
...
static const char *const mode_name[] = {
    ...
    "Dist PID",       // 23  хойд encoder-ээр зай яв: △→яв, ✕→зогс, L1/R1→зай ∓
 ...
 *    control_data[4][0] = "дараах" товч (Share),  control_data[4][1] = "сонгох"
 * (Options)
 *
...
void selectMode(void) {
  ...
  while (control_data[4][1] == 0) {
    ...
    LPMS_Read(); // сонголтын үед ч DMA буферээ хоослох (хуучирсан өгөгдөл
                 // хурааахгүй)
  ...
  case 21: // СОЛО рак тест (△/✕ front, ○/▭ back) + serial
    // mode_check_rack: switch хүрэхгүй гацвал (home хүрээгүй) timeout →
    // зогсооно.
    while (true) {
      mode_check_rack();
      Rack_Solo_Test();
  ...
  case 23: // хойд encoder-ээр ЗАЙ явах PID (△/✕, L1/R1)
    while (true) {
      Drive_Distance_Test();
    }

  case 24: // ЦЭНХЭР платформ траекторууд: △→go3 ○→go2 □→go1 ✕→зогс
    while (true) {
      Blue_Go_Test();
    }

  case 25: // ГАРАХ маневрын рак-сунах хэсэг (D-Up эхлүүл)
    while (true) {
      mode_check_rack();
      Exit_Test();
    }
 ...
 *    PWM : -1000..1000  (сөрөг = ухрах, эерэг = урагш; хэмжээ 1000-аар
 * хязгаарлагдана)
 *
...
void motor_control(uint8_t type, int PWM) {
  ...
  if (PWM < 0)
    PWM = -PWM; // хэмжээ (абсолют утга) авах
  if (PWM > 1000)
    PWM = 1000; // PWM дээд хязгаар
