...
void colorFill(SSD1306_COLOR color) {
 ...
 */
char printCh(char ch, FontDef Font, SSD1306_COLOR color) {
    uint32_t i, b, j;
...
void printStr(const char* format, ...) {
    ...
    for(size_t i = 0; i < strlen(buffer); i++) {
        printCh(buffer[i], Font_7x10, White);
    }
}
void printFloat(float num) {
    int int_part = (int)num;  // Бутархай тооны бүхэл хэсгийг авна
    float fractional = num - (float)int_part;  // Бутархай хэсгийг олно
    int frac_part = round(fractional * 1000);  // Бутархай хэсгийг 1000-р үржүүлж бүхэл тоогоор хувиргана

    if (num < 0 && fractional != 0) {  // Хэрвээ тоо сөрөг бөгөөд бутархай хэсэг нь 0-ээс их байвал
        int_part--;  // Бүхэл хэсгийг нэгээр бууруулна
        frac_part = 1000 - frac_part;  // Бутархай хэсгийг зөв тооцоолно
    }

    char buffer[128];
    sprintf(buffer, "%d.%03d", int_part, frac_part);  // Бүхэл ба бутархай хэсгийг нэгтгэн хэвлэх
    printStr(buffer);  // Энэ функц нь таны тодорхойлсон OLED хэвлэх функц байна
}
